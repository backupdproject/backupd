...
func (e *fakeEngine) issueCSRF(w http.ResponseWriter, r *http.Request) string {
	if cookie, err := r.Cookie("retnd_csrf"); err == nil && cookie.Value != "" {
		return cookie.Value
	}
	...
	e.csrf[token] = true
	e.mu.Unlock()
	http.SetCookie(w, &http.Cookie{Name: "retnd_csrf", Value: token, Path: "/"})
	return token
}

func (e *fakeEngine) signedIn(r *http.Request) bool {
	cookie, err := r.Cookie("retnd_session")
...
func (e *fakeEngine) login(w http.ResponseWriter, r *http.Request) {
	...
	e.mu.Unlock()
	http.SetCookie(w, &http.Cookie{Name: "retnd_session", Value: token, Path: "/"})
	w.WriteHeader(http.StatusNoContent)
}
