...
func TestRun_ArtifactsRefusesAnUnconfiguredFilter(t *testing.T) {
	...
	const (
		missingSet    = cliecho.Binary + ": app: no configured backup set named production/no-such-set"
		missingSource = cliecho.Binary + ": app: no configured source named no-such-source"
	)
...
func TestRun_ValidateAcceptsFlagsOnEitherSideOfItsOperand(t *testing.T) {
	...
	const arity = cliecho.Binary + ": validate takes exactly one argument: <source/backup-set/artifact>"
...
func TestRun_VersionRefusesWhatItDoesNotUnderstand(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantExit:   2,
			wantStderr: cliecho.Binary + ": version takes no arguments",
		},
		{
...
func TestRun_CatalogAcceptsFlagsOnEitherSideOfItsSubcommand(t *testing.T) {
	...
	const (
		missing = cliecho.Binary + `: catalog: missing subcommand (expected "rebuild")`
		unknown = cliecho.Binary + `: catalog: unknown subcommand "frobnicate" (expected "rebuild")`
	)
	...
	tests := []struct {
		...
		},
		{
			...
			wantExit:   2,
			wantStderr: cliecho.Binary + `: catalog: unknown subcommand "--dry-run" (expected "rebuild")`,
		},
		{
