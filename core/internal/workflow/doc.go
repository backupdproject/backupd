...
// link, its own mode must not let another account write it, no directory
// containing it may let another account replace it, it must be inside the
// approved root, and its basename must match ScriptNamePattern exactly. A
