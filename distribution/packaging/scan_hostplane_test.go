...
func TestScanForHostPlaneModificationCatchesEveryMarker(t *testing.T) {
	cases := []struct {
		...
		{"pve cluster filesystem", "deploy.yml", "x-hook: cp backupd.conf /etc/pve/local/\n"},
		...
		{"host systemd unit", "deploy.yml", "x-unit: /etc/systemd/system/backupd.service\n"},
		{"host service state", "deploy.yml", "x-post: systemctl enable backupd\n"},
