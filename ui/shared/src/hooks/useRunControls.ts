...
export function commandFor(scope: RunScope): string {
  return scope.kind === "all" ? "retnd run" : "retnd fetch --backup-set " + scope.id;
}
      ...
      request.then(
        ...
        (e: unknown) => {
          ...
          emitBrowserNotice({
            ...
            code,
            message: failure === null ? "retnd did not answer." : refusal.message,
            ...
            remediation:
              failure === null
                ? "The request got no reply at all, so whether the run started is unknown. Check that the retnd service is still running, then ask again."
                : refusal.remediation,
