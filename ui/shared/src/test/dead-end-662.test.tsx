 ...
 *     retrying will not help. Run retnd fetch --backup-set
 *     cicd-pipeline/var-backups from a shell on this host instead.
 *   $ retnd fetch --backup-set cicd-pipeline/var-backups
 *   [you] post /operations refused: DESTRUCTIVE_OPERATIONS_DISABLED
 *   # no retnd equivalent yet · POST /api/v1/operations
 *
 ...
 * prints "no retnd equivalent yet" and names the route that just refused.
...
describe("the remedy a blocked run offers, and the console it is offered in (#662 defect 4)", () => {
  it("does not tell the operator to run a command the same window calls nonexistent", () => {
    ...
    const remedy = "retnd fetch --backup-set " + SET_ID;
      ...
      },
      {
        ...
        event: event({
          fields: {
            ...
            command_gap_detail:
              "`retnd fetch` starts a cycle in your own shell, not in this engine"
          }
        })
      }
    ];
    ...
    expect(
      ...
      "#662 defect 4: the docked terminal offers `" +
        remedy +
        "` as the remedy and, four lines later, says the same operation has no retnd equivalent and maps to the " +
        ...
        text
    ).not.toContain("no retnd equivalent yet");
  });
  ...
  it("control: a refusal that offers no command carries no contradiction to find", () => {
      ...
      [
        {
          ...
          event: event({
            fields: {
              ...
              route: "POST /api/v1/operations",
              command_gap: "no retnd equivalent yet",
              command_gap_detail: "`retnd run` starts a cycle in your own shell, not in this engine"
            }
          })
        }
      ],
    ...
    expect(text).toContain("# no retnd equivalent yet · POST /api/v1/operations");
    expect(text).not.toContain("retnd fetch --backup-set");
  });
  ...
  function dockApi(events: SetActivityEvent[]): RetndApi {
    ...
    return { getLiveActivity: () => Promise.resolve(answer) } as unknown as RetndApi;
  }
   ...
   * left the whole suite green while the panel went on printing "no retnd
  ...
  it("does not print the contradiction on the panel either, which is the window the operator was reading", async () => {
    ...
    const remedy = "retnd fetch --backup-set " + SET_ID;
    const detail = "`retnd fetch` starts a cycle in your own shell, not in this engine";
    ...
    expect(await screen.findByText(/no retnd equivalent yet/)).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(
        screen.queryByText(/no retnd equivalent yet/),
        "#662 defect 4, on the surface it was reported from: the panel offers `" +
          remedy +
          "` as the remedy and still prints \"no retnd equivalent yet\" for the same operation, in the same window. " +
...
describe("what the browser offers an operator whose backup is stuck (#662 defect 3)", () => {
  function renderQuarantine(artifacts: BackupArtifact[], api: RetndApi = createMockApi()) {
  ...
  it("presses the FAILED backup's one control and it reaches the retry verb for that backup", async () => {
    ...
    // a real retnd in a container: `retry` exits 0 and prints "re-entering
