...
// CLI (core/cmd/backupd, unchanged by this issue) inside the same
...
// cmd/backupd-web, for the same reason `backupd` lives in
// cmd/backupd: a Go package path is not operator-visible, and
...
// stays on cmd/backupd: this binary is deliberately narrow rather
// than a second, competing CLI, since core/cmd/backupd cannot be
...
import (
	...
	"github.com/backupdproject/backupd/apps/generic/webui"
	"github.com/backupdproject/backupd/core/cliecho"
	"github.com/backupdproject/backupd/core/service"
)

// Set at build time with -ldflags (see container/Dockerfile), exactly
// like core/cmd/backupd's own version/commit vars.
var (
	version = "dev"
	commit  = "none"
)

// defaultConfigPath matches core/cmd/backupd's own default and
...
// core/service.StateDatabaseDefault, which core/cmd/backupd's own
...
// core/cmd/backupd publishes in its own usage block, so a caller
...
// which builds ./cmd/backupd from the core module and runs THAT;
...
func cmdServe(args []string) int {
	...
	// core/cmd/backupd/daemon.go's own convention exactly: this is
...
// run's own top-level switch, the same shape core/cmd/backupd's
...
//
// core/cmd/backupd carries the same four in its own setup.go, and
