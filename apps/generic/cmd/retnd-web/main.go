...
// than a second, competing CLI, since core/cmd/retnd cannot be
...
// like core/cmd/retnd's own version/commit vars.
var (
	version = "dev"
	commit  = "none"
)
...
// core/service.StateDatabaseDefault, which core/cmd/retnd's own
...
// core/cmd/retnd publishes in its own usage block, so a caller
...
// which builds ./cmd/retnd from the core module and runs THAT;
...
func cmdServe(args []string) int {
	...
	// core/cmd/retnd/daemon.go's own convention exactly: this is
...
// run's own top-level switch, the same shape core/cmd/retnd's
...
//
// core/cmd/retnd carries the same four in its own setup.go, and
