...
//
// The CLI's rename (0.3.3, core/cliecho/cliname.go) moved `rbm` to
// `backupd` and moved the web host to `backupd-web` everywhere AROUND this
// binary: the image ships /backupd-web, container/compose.yaml and all
// eleven adapter compose files run /backupd-web,
...
// itself, which went on printing `rbm-web:` in front of every
// diagnostic and `usage: rbm-web` at the top of its help. So a
...
//
// EPIC R (#885) renames the command again, `backupd` to `retnd`, which is
// why legacyName below now reads `backupd`: this check is about the name
// the binary must no longer print, and that name moves with each rename.
//
...
// that still mounts /etc/backupd. A check
...
// The occurrences of legacyName that are NOT a program naming itself and
// that no lexical rule can tell apart from one, pinned to the file they
// live in and to the issue that deletes them.
//
// `rbm` needed no list like this: it was a command spelling and nothing
// else, so "outside a path segment" was the whole rule. `backupd` is also
// the PRODUCT's name, and EPIC R renames the product on a different
// schedule from the command (FR-36: the display name everywhere a human
// reads it is phase 2's, 757 occurrences of it). These three are mail
// subjects -- the product introducing itself to an operator's inbox, in
// exactly the `name: thing` shape a diagnostic prefix also has -- so they
// are pinned rather than matched, and the pin carries the path so that
// the same sentence appearing in a NEW file is still a finding.
//
// TestTheExemptionsAreStillReal below fails when one of these stops
// matching, so the list cannot outlive what it excuses.
var notThisProgramNamingItself = []struct {
	file    string
	literal string
	owner   string
}{
	{"../../../common/auth/local/recovery.go", "backupd: password reset", "#892 (R2.2, the product's display name in prose)"},
	{"../../../common/auth/local/recovery.go", "backupd: SMTP test", "#892 (R2.2, the product's display name in prose)"},
	{"../../../common/auth/local/verify.go", "backupd: verify your recovery email", "#892 (R2.2, the product's display name in prose)"},
}

func exempt(path, literal string) bool {
	for _, e := range notThisProgramNamingItself {
		if filepath.ToSlash(path) == e.file && literal == e.literal {
			return true
		}
	}
	return false
	...
	for _, root := range selfNameRoots {
		err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
			...
			ast.Inspect(file, func(n ast.Node) bool {
				...
				if namesAnotherProgram(value) && !exempt(path, value) {
...
func TestNamesAnotherProgramTellsThePathsApart(t *testing.T) {
	...
	mustPass := []string{
		...
		// The right-hand anchors, and each one is an identifier EPIC R
		// keeps or retires on somebody else's issue. A program's name
		// is never continued by an underscore, and the only hyphen
		// compound that is a program here is `-web`.
		legacyName + "_session",
		legacyName + "_csrf",
		legacyName + "_workflow_runs_total",
		legacyName + "-workflow-runner.service",
	}
	for _, s := range mustPass {
		if namesAnotherProgram(s) {
			t.Errorf("%q was flagged; the rule has to leave the paths, the runtime identifiers and the image's own filenames alone or somebody will suppress it", s)
		}
	}
}

// TestTheExemptionsAreStillReal holds notThisProgramNamingItself to the
// tree. A pin that matches nothing is a line excusing something that is
// no longer there, and the next person to read the list has no way to
// tell that from a live exemption.
func TestTheExemptionsAreStillReal(t *testing.T) {
	for _, e := range notThisProgramNamingItself {
		data, err := os.ReadFile(e.file)
		if err != nil {
			t.Errorf("exemption for %q names %s, which cannot be read: %v", e.literal, e.file, err)
			continue
		}
		if !strings.Contains(string(data), e.literal) {
			t.Errorf("%s no longer contains %q, so the exemption owned by %s is excusing nothing; delete the line", e.file, e.literal, e.owner)
		}
	}
}

// namesAnotherProgram reports whether s spells the previous command name
// as a program rather than as something else that carries the same word.
//
// Two anchors, because `backupd` is a word with four jobs in this tree
// and only one of them is the command (cliname.go argues each). On the
// left, a `/` makes it a path component: /etc/backupd, /backupd-web. On
// the right, `_` makes it a runtime identifier (backupd_session, the
// fourteen backupd_* metric series) and a `-` followed by anything other
// than `web` makes it a deployment name (backupd-workflow-runner.service,
// the compose service, the container). R1.4 (#889) and R1.5 (#890) own
// those two populations and both deliberately keep some of them for a
// release, so a rule without the right-hand anchor would be a rule this
// epic has to suppress halfway through -- and check-brand-drift.sh, which
// scans the same word across the whole tree, reaches the same conclusion
...
func namesAnotherProgram(s string) bool {
	for i := 0; ; {
		...
		at := i + j
		end := at + len(legacyName)
		rest := s[end:]
		pathComponent := at > 0 && s[at-1] == '/'
		identifier := strings.HasPrefix(rest, "_")
		otherCompound := strings.HasPrefix(rest, "-") && !strings.HasPrefix(rest, "-web")
		if !pathComponent && !identifier && !otherCompound {
			return true
		}
		i = end
	}
