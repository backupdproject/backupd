...
CasaOS, so nothing here replaces earlier packaging.
...
environment values and volumes are presented in CasaOS's own install dialog. It
...
A CasaOS store install deploys the file as it stands, with no `.env` beside it.
...
and every one of them is CasaOS's own layout: private application data under
...
Then install through **App Store, Custom Install** with this file, or submit it
to the CasaOS AppStore. The engine prints a one-time enrollment link on first
start; read it in the container log.

The full operator procedure, including update, removal and the evidence that
retained backups survived, is
[`docs/acceptance/casaos-app-store-install.md`](../../docs/acceptance/casaos-app-store-install.md).
Nothing in it has been executed: it needs a CasaOS host, and no criterion in it
is ticked.
...
## What CasaOS does not give this product, stated rather than emulated

CasaOS has a user account of its own. This app does not use it: sign-in is the
product's own local account, over its own session cookie, and no CasaOS identity
...
Compose deployment, which under CasaOS it is.

The reason it is not a CasaOS-branded bridge is measured rather than assumed. A
...
CasaOS import may enter either.

## CasaOS and ZimaOS are one runtime and two registrations

`apps/zimaos/` holds the same services, the same paths and the same `x-casaos`
