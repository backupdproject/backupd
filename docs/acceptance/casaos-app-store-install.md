...
carrying an `x-casaos` block, so `apps/casaos/compose/backupd.yml` is both
...
1. In CasaOS, **App Store, Custom Install**, choose **Import** and paste
   `apps/casaos/compose/backupd.yml`, or submit the same file to the
...
| `/DATA/AppData/backupd/secrets/workflow-runner.token` | `/etc/backupd/workflow-runner.token` (read-only) | the credential the engine presents |
