...
```bash
ssh-keygen -t ed25519 -f /etc/retnd/ssh/backup_key -C "retnd" -N ""
```

The empty `-N ""` means no passphrase. That's deliberate, not an oversight:
retnd runs unattended and has nowhere to prompt for one. The
...
```bash
chmod 600 /etc/retnd/ssh/backup_key
chown root:root /etc/retnd/ssh/backup_key
```
...
```bash
useradd --system --create-home --home-dir /srv/retnd \
        --shell /usr/sbin/nologin backupsvc
mkdir -p /srv/retnd/incoming
```
...
```bash
mkdir -p /srv/retnd/incoming/.ssh
chmod 700 /srv/retnd/incoming/.ssh
cp /etc/retnd/ssh/backup_key.pub /srv/retnd/incoming/.ssh/authorized_keys
chmod 600 /srv/retnd/incoming/.ssh/authorized_keys
chown -R backupsvc:backupsvc /srv/retnd/incoming/.ssh
```
...
Match User backupsvc
    ChrootDirectory /srv/retnd/incoming
...
```bash
chown root:root /srv/retnd /srv/retnd/incoming
chmod 755 /srv/retnd /srv/retnd/incoming
```
...
```bash
mkdir -p /srv/retnd/incoming/backups
chown backupsvc:backupsvc /srv/retnd/incoming/backups
chmod 750 /srv/retnd/incoming/backups
```
...
chown produceruser:backupsvc /srv/retnd/incoming/backups/some-artifact.dump.zst
chmod 440 /srv/retnd/incoming/backups/some-artifact.dump.zst
```
...
`retnd` process can delete a stale backup once it's confirmed
...
Then, from wherever retnd will actually connect from:

```bash
ssh-keyscan -t ed25519 -p 22 production.example.internal > /etc/retnd/known_hosts
ssh-keygen -lf /etc/retnd/known_hosts
```
...
once they match does `/etc/retnd/known_hosts` mean anything.

From here on, retnd itself is what keeps this honest: if that host's
...
retnd at is a separate decision, and there are three ways to make
it:
...
two is a config error retnd refuses at startup, not a precedence
...
retnd's own memory at all: rclone opens the file itself, reads it,
...
inherited from retnd's own process), so the command has to be
...
through an environment variable retnd would otherwise have to trust
it with.

Both of these put the resolved key in retnd's memory for the
duration of one connection attempt, which is exactly the cost `key.file`
avoids. In exchange, retnd validates what came back before it goes
...
that named as the problem: retnd runs unattended, and there is
...
passphrase-protected production key to get it into a form retnd
could use, and the resulting plaintext file sat on a volume also reachable
over an SMB/AFP share, entirely outside retnd's own permission
model.
...
- It defends the key FILE against being read directly: disk theft, a copy
  taken by a backup of retnd's own state directory, and -- the
...
- It does NOT defend a live process's memory. Authenticating a connection
  still requires the plaintext key in retnd's own memory for the
  ...
  dump, or a debugger attached to a running retnd can still reach
...
- **It does NOT defend anything if the encryption key (the `key_encryption`
  ...
  retnd's own runtime, or a secrets manager via `command`.
...
connection" step) once `key_encryption` is set: retnd detects the
...
`ssh-keygen` produces; an encrypted one begins with retnd's own
...
retnd's own configured `key_encryption` source can read it.
    ...
    backup_sets:
      - id: postgres-primary
        remote:
          ...
          port: 22
          user: backupsvc
          key_file: /etc/retnd/ssh/backup_key
          known_hosts: /etc/retnd/known_hosts
        remote_path: /backups
```
...
Mount `backup_key` and `known_hosts` read-only into wherever retnd
...
nothing to mount for either, since the key never lives in a file retnd
...
config asks for, not something retnd decides on its own.
...
retnd stays under such a limit on its own: every operation it
...
are in flight *inside* one connection (retnd pins that at 64, and it
...
with the same files retnd will use:

```bash
sftp -i /etc/retnd/ssh/backup_key \
     -o UserKnownHostsFile=/etc/retnd/known_hosts \
...
step 5, since retnd itself will fail the same way for the same
reason.
...
- More than one key source configured at once. Two is a mistake to fix, not
  a precedence order retnd guesses through.
...
right posture for a backup source and is exactly what retnd's own
machine-tier fixture runs against.

That has a direct consequence for EPIC L's remote hooks. A
`NAME.remote.sh` runs on this source host, and retnd does not assume
  ...
  exec_connections:
    - id: db-hooks
      remote:
        type: sftp
        host: db.example.com
        user: retnd-hooks          # NOT the transfer account
        known_hosts: /etc/retnd/known_hosts
        key:
          file: /etc/retnd/hooks_ed25519
...
narrowest privileges the hooks actually need. retnd runs a hook **as
...
What retnd guarantees about a remote hook, and what it does not:
...
- The account's own shell startup files are that account's trust
  ...
  exports for that account is read by bash before anything retnd sends
  can clear it, so a connection whose session arrives with `BASH_ENV` or
  `ENV` set is refused rather than run.
- On a timeout or a cancellation, retnd asks the remote side to stop
