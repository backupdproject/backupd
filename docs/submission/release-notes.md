...
- The same operation is on the API (`DELETE /api/v1/backup-sets/{source}/{set}`) and on the
  command line (`backupd backup-set remove <source/backup-set>`). Against a server that
...
- One engine per deployment. Starting a second `backupd daemon`, or a second web host,
