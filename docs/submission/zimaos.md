...
The submission bundle for ZimaOS app store, in the form that store asks for it. The shared materials
...
store compose file under `apps/zimaos/`.
...
`x-casaos` block, so `apps/zimaos/compose/retnd.yml` is both the runtime
...
is a step of `docs/acceptance/zimaos-app-store-install.md` rather than a check here.
...
ZimaOS app store asks for 2. See `docs/submission/screenshots.md` for what to capture and
`docs/acceptance/store-submission-preflight.md` for the procedure that captures them on real
hardware.

## The store icon

`apps/zimaos/icon.svg` is the in-app mark the `x-casaos` block points at, and
...
- [ ] The acceptance run in `docs/acceptance/zimaos-app-store-install.md` has been done on
      real hardware and accepted.
