...
| 6 | `RCLONE_MANAGER_[A-Z]*` — incl. `RCLONE_MANAGER_SOURCE_PORT` (installer, documented in `docs/install.md`), `RCLONE_MANAGER_MACHINES_NETWORK`, and `RCLONE_MANAGER_UNIT` heredoc sentinels | `RETND_*` | main | 19 / 48 | **R1.4** (installer sites with **R1.5**, the prose with R2.2) | guard, since #887 | The hole this inventory found: none of the guard's four patterns matched `RCLONE_MANAGER_`, so a first-brand env prefix had sat green for two renames — and one of them is operator-facing. #887 added the pattern and put all 30 surviving names on `pending` |
...
| 19 | `rclone-manager`, `rclone_manager`, `backup-manager`, `backup_manager` as prose and identifiers | `retnd` | both | ~60 / ~190 | **R2.2** (prose), **R1.4** (identifiers), **R1.2** (guard lists) | guard, since #887 added the patterns | The unfinished half of two earlier renames. `CHANGELOG.md` and the dated `docs/design/` notes keep theirs |
