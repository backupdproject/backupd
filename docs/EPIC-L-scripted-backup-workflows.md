...
3. **The end-to-end suite is not all green, and exactly one red belongs to this epic.** The definitive run is **208 passed, 29 failed, 75 skipped** in 30.2 minutes, on backupd-tests pin `859be9631a2d9da99996b6515371d9b82f6ce81b`, executed by `scripts/e2e/three-machine-web-ui.sh` against a product image built from `b4254ff1` — L9's branch rebased onto this document's own base, `e9e13495`. **28 of the 29 failures are pre-existing product/suite drift tracked in #913** — proven pre-existing by running the same rig with `--no-workflows` against both this pin and the previous one (`d0431449`) and getting byte-identical failure lists. No workflow case is blocked behind an issue number. L9 #816's PR (#914) carries the per-case status and is authoritative; if it and this paragraph ever disagree, that PR is right and this paragraph is stale, which is the failure mode a number copied into a second document always has and the reason the pointer stays here beside the number rather than instead of it.
   ...
   **The recovery path is proved at the API level and NOT through the browser, and that is the one red this epic owns.** Killing the engine while a step reports `running` yields the two holds it should — one at `global` scope, one at `set` scope, five seconds after the engine is back — and the rig asserts that. The browser case that drives both ways out of a hold is the only non-#913 failure in the final run, and it fails *before it crashes anything*: the backup set it crashes into had stopped being scheduled, so the case never got a live hook to interrupt. The scheduler itself is healthy — that set was measured starting a run every ~77 seconds on a live stack — so the cause is a hold left unsettled earlier in the suite starving it, which is a suite-sequencing defect being routed separately.
