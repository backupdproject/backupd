...
- [x] The web UI draws the three verdicts without deriving one from another, and offers exactly the two exits from a hold.
...
## What has not been proven, and the four carve-outs
...
2. **Remote hooks had never actually run until the end of this epic (#919, fixed and merged as #920).** L9's rig found it: the engine passed a step id containing a tilde as the remote-exec token, `remoteexec`'s token rule refused it *before any SSH session was opened*, and the catch-all mislabelled that refusal as `transport_lost`. So from L2/L3 until #920, no remote hook had executed on any path — while the token-less capability probe went on reporting `exec_capability ok`, which is exactly why the unit tests did not catch it. The fix is a dedicated token derivation. The lesson belongs in this document rather than only in a commit message: a capability probe that does not exercise the same code path as the operation it is a probe for is a green light for an untested path.
3. **The end-to-end suite is not all green, and none of the reds are workflow reds.** The workflow scenarios pass. The web-UI suite around them carries a set of pre-existing product-to-suite drift failures unrelated to this epic, tracked in **#913** and triaged separately; they were proved pre-existing by running the same rig with workflows disabled against both this branch and the previous pin and getting byte-identical failure lists, so no pin bump moves them. The exact counts, the pin they were measured on, and the per-case status are recorded in L9 #816's PR rather than restated here, because a number copied into a second document is a number that goes stale in one of them.
4. **Some shipped deployment profiles advertise local hooks and mount none of the paths a local hook needs (#921, open).** FR-39's answer only exists if the deployment actually mounts the workflow root read-only, the runtime directory holding the runner socket, and the runner credential file. The canonical definition does; several adapter profiles declare the capability while mounting none of the three, which `distribution/packaging`'s adapter-equivalence check sees as a divergence from the canonical stack. That is open as **#921** and being fixed separately; until it lands, "local hooks work on this platform" is a per-profile question and not a product-wide claim. It is the narrower, checkable half of what GC-22 (#877) is about, and GC-22 stays unticked.
