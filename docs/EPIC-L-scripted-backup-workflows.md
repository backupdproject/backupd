...
**FR numbering:** this specification continues the product's FR series at **FR-36**. FR-1 through FR-24 are defined in `docs/EPIC.md`; FR-26 is claimed by the `version` command in `core/cmd/backupd` and `core/internal/app`; FR-27 through FR-35 are EPIC E's, in `docs/EPIC-E-alternative-storage.md`. FR-25 is an unclaimed hole and stays one. Nothing here renumbers an existing FR.
...
| CLI | `core/cmd/backupd/{workflow.go,workflowrunner.go,settingsworkflow.go,backupsetworkflow.go,validateworkflow.go}` | presentation |
...
Precedence is `sanitized baseline < workflows.environment < backup-set environment < BACKUPD_* built-ins`. The first three are ordinary specificity. The fourth is different in kind: the built-ins are the run's own facts, so the whole `BACKUPD_` prefix is refused *in configuration, at validation time* rather than overridden at merge time — a key an operator can write and this product silently discards is a key that looks like it works.

Nineteen built-ins are supplied: `BACKUPD`, `BACKUPD_RUN_ID`, `BACKUPD_BACKUP_SET_ID`, `BACKUPD_BACKUP_SET_NAME`, `BACKUPD_PHASE`, `BACKUPD_STEP_ID`, `BACKUPD_STEP_NAME`, `BACKUPD_STEP_TARGET`, `BACKUPD_SOURCE_HOST`, `BACKUPD_SOURCE_PATH`, `BACKUPD_DESTINATION`, `BACKUPD_WORK_DIR`, `BACKUPD_BACKUP_STATUS`, `BACKUPD_WORKFLOW_STATUS`, `BACKUPD_CLEANUP_STATUS`, `BACKUPD_BACKUP_ERROR_CODE`, `BACKUPD_STARTED_AT`, `BACKUPD_RECOVERY`, `BACKUPD_CLEANUP_REASON`. A before hook is told `BACKUPD_BACKUP_STATUS=unknown` — a real value, because an unset variable and one saying "nobody knows yet" read identically in `test -z`.
...
A `.local.sh` hook means "run this on the machine backupd is installed on". The engine cannot run one: its image is distroless and has no shell, the container is read-only and non-root, and every capability is dropped. That contract does not move to buy a shell.

Local hooks execute through a **Host Workflow Runner**: a separate host process, built from the same commit as the engine and extracted from the same image by the installer, supervised as `backupd-workflow-runner.service`. It listens on one Unix-domain socket and never a TCP port, requires an installation-scoped credential on every connection, refuses an engine whose version is not exactly its own, and refuses to run as root. It accepts captured script *bytes*, never a path. Each hook runs in an ephemeral container with `--cap-drop ALL`, `--security-opt no-new-privileges`, `--read-only`, a non-root `--user`, `--network none` and a pids limit, and no Docker socket is mounted into a hook container under any name. The runner never pulls: an image that is not present is a refusal, not a download.
...
Everything the browser can do to a workflow, a terminal can do. `backupd workflow run list|show|steps|log`, `backupd workflow recovery show|resume-cleanup|acknowledge`, `backupd settings workflow [patch|env …]`, `backupd backup-set workflow [patch|env …]`, `backupd validate workflow <source/backup-set>`, `backupd workflow-runner serve|status`. Every mutating route has a `core/cliecho/routes.go` builder or an accepted parity exemption, enforced by the parity test in `apps/common/webhost/router_test.go`.
...
The bypass is the single exception and its boundary is the administrator boundary. `backupd fetch --skip-workflow-scripts` is declared **only to be refused**, because that command's pass runs no hooks at all and a flag that silently does nothing is worse than one that says so. The only bypass is `skip_workflow_scripts` on `POST /api/v1/operations` with `run_backup_set`, it is recorded durably on the run row and in a warn-level event naming who asked, and it cannot clear a recovery hold.
...
- [ ] The end-to-end suite is green on the container rig. Partly: see section 8, which says exactly what is green, what is red, and why.
...
- [x] **§3.3 Every new CLI verb reaches the dispatch table and gets a reference-page row.** `workflow` and `workflow-runner` are both in `core/cmd/backupd/main.go` and both have rows. *(Gated:* `distribution/packaging/site_reference_test.go`.*)*
...
3. **The end-to-end suite is not all green, and the workflow reds are narrower than the number looks.** The definitive run is **206 passed, 31 failed, 76 skipped** in 43.1 minutes, on backupd-tests pin `b4f62eb83b68e4fdc4427bf9f0567a2cc24a1ff0`, executed by `scripts/e2e/three-machine-web-ui.sh` against a product image built from L9's branch with #920 in it. **28 of the 31 failures are pre-existing product/suite drift tracked in #913** — proven pre-existing by running the same rig with `--no-workflows` against both this pin and the previous one (`d0431449`) and getting byte-identical failure lists. L9 #816's PR (#914) carries the per-case status and is authoritative; if it and this paragraph ever disagree, that PR is right and this paragraph is stale, which is the failure mode a number copied into a second document always has and the reason the pointer stays here beside the number rather than instead of it.

   Two edges of that evidence are not to be rounded off, because rounding them off is how a release gate becomes a formality.

   **Remote hooks do run, and one assertion about them was still red for a reason that proves they ran.** The after-stage case — a remote run finishes the cleanup its after stage owes — is green. The before-stage case failed only on a cross-check comparing names: the hook printed that it had run on `exechost` while the product reported its executor as `Remote · exec-host`. Two names for one machine, in the rig's own fixture rather than in the product. That has been corrected on L9's branch — container hostname, network alias, connection host and connection id are now all `exechost` — and the correction is **unverified by a run** at the time of writing, because confirming it alone would have cost a full rig cycle for a result the next full run gives free.

   The instructive part is not the defect, it is how it was told apart from a product failure. The assertion that failed quoted *both* strings, so its own message read as "the hook printed that it ran on `exechost` and the product says it executed on `Remote · exec-host`, so one of them is describing a different machine" — which is what made it obvious the hook had executed and the fixture was wrong. A weaker assertion that merely checked the line contained some hostname would have gone green while proving nothing; a lazier one comparing against a literal would have failed without saying why. Assertions that print their evidence are the reason this was a ten-minute diagnosis instead of a hunt through the product.

   **The recovery path is proven at the API level and not yet through the browser.** Crashing the engine mid-hook produces the two holds it should, one at `global` scope and one at `set` scope, and that is asserted by the rig. The two browser cases that drive `resume-cleanup` and `acknowledge` were red in this run and are being fixed suite-side. So FR-42 rests on the unit and API evidence in §8 above, and the claim "an operator can resume or acknowledge from the web UI" rests on the UI tests in L7/L8 rather than on an end-to-end run. That is a real gap in this epic's release gate, it is somebody's next task, and it is not closed by this document.
