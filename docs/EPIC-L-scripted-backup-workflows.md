...
3. **The end-to-end suite is not all green, and none of the reds are workflow reds.** The workflow scenarios pass. The web-UI suite around them carries a set of product-to-suite drift failures unrelated to this epic, tracked in **#913** and triaged separately; they were proved pre-existing by running the same rig with workflows disabled against both this branch and the previous pin and getting byte-identical failure lists, so no pin bump moves them.

   The last completed run at the time of writing, on tests pin `b79c28d`, was **204 passed, 31 failed, 78 skipped**, of which **28** of the 31 are the #913 drift. That number is quoted rather than gestured at because a document that says only "some failures are pre-existing" cannot be reconciled with a later run by anybody; and it is dated and pinned rather than stated flatly because L9 #816's rig was still being re-run when this was written. **L9 #816's PR carries the authoritative counts, the pin they were measured on and the per-case status.** If it and this paragraph disagree, that PR is right and this paragraph is stale — which is the failure mode a number copied into a second document always has, and the reason the pointer stays here beside the number rather than instead of it.
