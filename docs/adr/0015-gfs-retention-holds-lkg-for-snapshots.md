...
committed manifest, and a run whose snapshot has already been deleted. A
hold that is accepted and protects nothing is worse than no hold, because
somebody reads the list and stops worrying.
...
### 4. The engine's own retention is turned off, deliberately and at open
...
`core/internal/backupengine/kopia/enginepolicy.go` stores a global policy
that keeps everything (six explicit zeros, which is the vendor's own
spelling for "no count applies": `EffectiveKeepLatest` reads that as
`MaxInt`). It runs once per repository, at open, because that is the one
place every path that could write or checkpoint a snapshot passes through,
including repositories created by an earlier build. A repository whose
policy is not neutral and whose storage will not accept the correction is
refused rather than opened: opening it means accepting that the engine may
delete this product's snapshots during the next long backup.
