...
- `cmd/retnd/main.go` is 25 lines and understands exactly one subcommand,
...
at a snapshot-capable volume, schedule snapshots independently of `retnd`), which
belongs in deployment documentation, not in `internal/`. Revisit only if a requirement
appears for `retnd` itself to trigger a snapshot (e.g. shell out to a NAS vendor
...
(`retnd status --prometheus > some.prom` on a cron, no HTTP server, no long-running
...
new call site anywhere else in the repository. Wiring it into a `retnd status
