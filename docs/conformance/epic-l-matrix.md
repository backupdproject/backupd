...
| GC-20 | No resolved secret reaches a metric label, an API response or CLI output | — | L6 (#813) owns these assertions and they land on `feat/813-workflow-surfaces`: `core/internal/metrics:TestRender_NoLabelCarriesASecretOrAPath`, `core/service:TestASecretReferenceIsStoredAndReportedAsALocationOnly`, `apps/common/webhost:TestNoWorkflowResponseCarriesAResolvedSecret`, `core/cmd/backupd:TestWorkflowEnvPrintsTheLocationOfASecretAndNeverItsValue` and `TestWorkflowEnvCarriesNoFieldAResolvedSecretCouldLiveIn`. This row becomes PASS with their proof references when the two branches meet; the plan-and-spool half is already proved here by `core/internal/workflow:TestNoResolvedSecretReachesThePlanTheHashOrTheSpool` | BLOCKED |
| GC-21 | Only an administrator with configuration authority may change workflow directories, ENV, secret references or the bypass, and every such mutation is audited without recording a credential | — | The mutating workflow routes and CLI verbs are L6 (#813)'s surface and do not exist on this branch; the audit rendering's own no-credential rule is already proved here by `core/internal/remoteexec:TestAuditCarriesEveryFactAndNoCredential`. This row becomes PASS against L6's authorization and action-log tests | BLOCKED |
