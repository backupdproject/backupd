...
**FR numbering:** this specification continues the product's FR series at **FR-27**. FR-1 through FR-24 are defined in `docs/EPIC.md`; FR-26 is claimed by the `version` command in `core/cmd/retnd` and `core/internal/app`. Nothing here renumbers an existing FR.
...
storage_mediums:
  - id: offsite_s3
    ...
    credentials:
      ...
      # command: ["op", "read", "op://infra/retnd/s3-offsite"]
```
...
- Credential files live under private state (`/var/lib/retnd`, EPIC B section 19.1), never under the backup root, and the API import flow mirrors SSH key import: the secret goes into private state, and the config holds a path.
...
- The CLI mirrors the same vocabulary (`retnd artifacts`, artifact detail), so a terminal operator and a UI operator read the same truth.
