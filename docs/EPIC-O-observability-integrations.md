...
1. The draft proposed "an opt-in structured-JSON log mode". **This product has no other mode.** `obs.New` (logger.go) writes newline-delimited JSON through `log/slog`'s JSON handler, unconditionally, and both production wirings (`core/cmd/backupd/setup.go:402`, `core/service/service.go:471`) point it at `os.Stdout`. Shipping a "JSON mode" toggle would add a knob whose off position does not exist, and §5's tooltip rule ("a field whose effect cannot be stated from the code gets no entry") would forbid explaining it. The draft was describing work that is already done and missing the work that is not.
...
5. The draft claimed "no migration needed" without checking. Nothing here is journal state, so that happens to be right, but it has to be asserted: `core/migrations/` is at `0013_workflow_lifecycle.sql`, and the claim EPIC O must make is that it stays there.
...
- **no migration**: `core/migrations/` stays at `0013`, asserted by the exit gate, and `core/tests/compat/capture_*.go` captures are updated because config and CLI output both move.
...
| 4 | Log wiring | `core/cmd/backupd/setup.go:402` and `core/service/service.go:471`, both `obs.New(os.Stdout, obs.LevelFromEnv())`. Level from `BACKUPD_LOG_LEVEL`, with `BACKUPD_DEBUG=1`/`RM_DEBUG=1` shortcuts | Level is environment-only: not in `config.yaml`, not in the browser, not in `api/v1` |
...
| 11 | Deployment | `container/compose.yaml` canonical (`LISTEN_ADDR: ":8080"`, distroless, read-only, non-root, `cap_drop: ALL`), embedded by `scripts/install/embed_compose.py`; 11 providers; 11 compose files across seven of them, plus 2 Unraid templates and 1 TrueNAS catalog template; `distribution/packaging/*.json` derived; `scan_endpoints.go` fails on committed credential/endpoint shapes | No Prometheus, Grafana, Loki or Promtail example anywhere |
| 12 | Gates | 16 `ci.yml` jobs; `release-gate` with a `needs` list held by `scripts/tests/release-gate-covers-every-job.test.sh`; `scripts/ci-local.sh` is the local mirror and the pre-commit hook; `core/migrations/` at `0013_workflow_lifecycle.sql`; next ADR is `0023` | No observability job, no dashboard guard |
...
6. `core/migrations/` is at `0013_workflow_lifecycle.sql`, and EPIC O's claim is that it stays there.
...
17. **Config compat**: five new fixtures (one full, one omitting the block, three invalid), `core/tests/compat/capture_*.go` updated, `core/migrations/` still at `0013`, and a config that never mentions `observability:` is byte-identical and behaves as it did.
...
| 7 | Storage, state and migrations | O1.5 | **No migration.** Nothing here is journal state; `core/migrations/` stays at `0013`, asserted, and the compat captures are updated |
...
| Does the journal schema change? | No. `core/migrations/` stays at `0013` |
