...
This FR is recorded as an ADR: [`docs/adr/0023-moving-the-repository-coordinates-once-and-last.md`](adr/0023-moving-the-repository-coordinates-once-and-last.md) carries the sequence, what the redirect covers and what it does not, the four references re-established by hand, the org-level carry-over, the rollback window, and a table of which half of the cutover has actually happened. R2.2 (#892) wrote it; R2.5 (#895) performs the transfer it describes.
...
- [x] `docs/site/reference.html`'s command table matches the dispatch table; all **six** site pages, their `<title>`s and the wordmark say `retnd`; all 55 `docs/site/screens/` files are re-recorded through the **five** capture scripts against `createMockApi` with the clock pinned, and the landing PR names the pin the Playwright was borrowed at. (Written as five pages and 44 files through four scripts: `docs/site/workflows.html` and its ten `wf-*.gif` clips landed in #925 after this line was written, and R2.3 swept the sixth page and re-recorded its clips rather than leaving one page of the site saying the old name.)
...
- [x] The site's "What has not been proven" section states which surfaces were re-captured, which store screenshots still show the old name, and that no provider store listing has been re-reviewed.
...
- [x] `NOTICE`, the licence inventory and `provenance/**` are regenerated forward; no already-published record is rewritten.
...
- [x] The CHANGELOG `[Unreleased]` entry names the issue, what changed, why, and what an existing deployment sees, including the one-release windows and the double-count caveat.
- [x] `docs/conformance/epic-r-matrix.md` has no `BLOCKED` row, and every `PASS` row's falsification has been run and watched to fail.
- [x] The deprecation-window-close issue is filed, naming every alias list entry it deletes, with the removal release from FR-43's table.
...
- [x] No surviving reference to the `backupdproject` organisation or the old repository names anywhere in the tree, outside a redirect-backed link that is deliberately left as history; the guard's fourth pattern goes red on a new one.
