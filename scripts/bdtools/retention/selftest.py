...
unrelated reason from reading as a pass. `scripts/bdtools/conformance/`
and `scripts/bdtools/compat/` (once ported) are the models and this is
...
`python3 scripts/bdtools/retention/selftest.py --check-anchors` is that
...
the copy a mutation is planted into is a real, complete working tree
  ...
  hazard in python: GONE from THIS file, by construction rather than by
                     ...
                     `scripts/bdtools/` consumes, so a second, private
...
APPLY_DELETES_NOTHING_NEW = (
    "\t\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): an apply\n"
...
SYMLINK_FOLLOWED_NEW = (
    "\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): stat through\n"
...
LAST_KNOWN_GOOD_NEVER_DECIDED_NEW = (
    "\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): FR-19 finds the\n"
...
LAST_KNOWN_GOOD_COPY_NOT_CONFIRMED_NEW = (
    "\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): the FR-30\n"
...
APPLY_TIME_LAST_KNOWN_GOOD_NOT_RECONFIRMED_NEW = (
    "\t\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): the apply\n"
...
ACKNOWLEDGE_NOT_REQUIRED_NEW = (
    "\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): the\n"
...
COMPARISON_CANNOT_FAIL_NEW = (
    "func retentionEvidenceCompare(before, after map[string]retentionEvidenceEntry, "
    "wantRemoved []string) []string {\n"
    "\t// PLANTED VIOLATION (scripts/bdtools/retention/selftest.py): the whole\n"
