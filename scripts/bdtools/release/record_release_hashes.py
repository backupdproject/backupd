...
`scripts/bdtools/release/record_release_hashes_guards.py`, which builds a throwaway
...
is inert unless something inspects `.returncode`.
  ...
  COMMIT does not name a commit in this repository
    ...
    held by:          record_release_hashes_guards.py "COMMIT that names no commit here"
  ...
  COMMIT is not HEAD
    hazard in python: STILL EXISTS if ported naively: `subprocess.run`'s
                       ...
                       to an empty string.
    held by:          record_release_hashes_guards.py "COMMIT that resolves but is not HEAD"
  ...
  the working tree is dirty in a build path
    ...
    held by:          record_release_hashes_guards.py "a working tree dirty in a path the
  ...
  REACHABLE_FROM cannot be resolved
    ...
    held by:          record_release_hashes_guards.py "REACHABLE_FROM that does not resolve"
  ...
  bit us)
    ...
    held by:          record_release_hashes_guards.py's paired "git exiting 128 is
                       reported as undecidable, not as a no" and its
                       exit-1 control

  OUT names a path relative to the toplevel, and every message echoes it
  verbatim (found reconciling the two ports, #697)
    hazard in bash:   none. The bash did `cd "$(git rev-parse
                       --show-toplevel)"` and then took
                       `OUT="${OUT:-container/release-manifest.json}"`,
                       so `mkdir -p`, `cat >` and both messages all used
                       the OUT string as given.
    hazard in python: LIVE, and this port had it: building the default as
                       `root / "container" / ...` makes every message an
                       absolute path. The operator-facing line stops
                       being the repo-relative path docs/deployment.md
                       quotes and a human can `git status`, and becomes a
                       /private/var tmpdir path when the guard suite runs
                       it. Closed by `_out()`, which returns the string
                       to PRINT and the path to WRITE separately.
    held by:          record_release_hashes_guards.py's two "Would write
                       container/..." assertions, plus its third arm
                       asserting an explicit absolute OUT is echoed
                       absolute -- the pair is what makes the check
                       non-vacuous in both directions.

  the exit status survives the bash shim
    hazard in bash:   none; there was no shim.
    hazard in python: LIVE. `scripts/release/record-release-hashes.sh` is
                       now a stand-in that runs this module, and a
                       wrapper that ran it without `exec` (or swallowed
                       the status) would flatten all five exit-2
                       refusals to 0 while still printing the refusal
                       text -- green output, no check. Closed by `exec`.
    held by:          every `expect()` call in
                       record_release_hashes_guards.py, which drives the
                       SHIM and asserts the code before the message.
...
def _out(root: Path, env: dict[str, str], default: str) -> tuple[str, Path]:
    """(what to print, where to write), reproducing the bash exactly.

    The bash did `cd "$(git rev-parse --show-toplevel)"` and then took
    `OUT="${OUT:-container/release-manifest.json}"`, so every message it
    printed and every path it wrote was the OUT string VERBATIM, relative
    to the toplevel unless the caller passed an absolute one. Resolving it
    to an absolute path here instead would change the operator-facing
    message from the repo-relative path docs/deployment.md quotes and a
    human can `git status` into a /private/var tmpdir path, which is what
    the guard suite's two `Would write container/...` assertions caught.
    """
    shown = env.get("OUT") or default
    return shown, root / shown


def run_guards(root: Path, env: dict[str, str]) -> tuple[str, str, str, Path, list[str], bool]:
    """The five reproducibility guards, in the same order as the bash.

    Returns (version, commit, shown_out, out, arches, unsafe) once every
    guard has passed, or exits 2 through `refuse`.
    """
    ...
    if unsafe:
        shown, out = _out(root, env, "container/.generated/release-manifest.local.json")
        ...
        return version, commit, shown, out, arches, unsafe

    shown, out = _out(root, env, "container/release-manifest.json")
...
# Byte-identical to the `note` already committed in
# container/release-manifest.json, deliberately. It is the operator's
# regeneration instruction, it names the path the docs and the shim name
# (`scripts/release/record-release-hashes.sh`, not this module), and a
# regeneration has to leave this field unchanged or every manifest diff
# carries a spurious hunk that hides the hashes that actually moved.
NOTE = (
    "Regenerate with scripts/release/record-release-hashes.sh, and only at a commit that is "
    ...
    "which is why the script now refuses to record a commit that is not an ancestor of origin/main and why "
...
def body(root: Path, version: str, commit: str, shown: str, out: Path, arches: list[str], unsafe: bool) -> int:
...
def resolve_toplevel() -> Path:
    ...
    by scripts/bdtools/tests/record_release_hashes_guards.py, or a real
    release checkout), never about the checkout `bdtools` happens to live
    in. Resolving the target from `__file__` instead is the defect this
    port was caught on: every guard fixture was silently checked against
    the developer's real workspace, printing plausible refusals about the
    wrong repository."""
    try:
...
def main(argv: list[str]) -> int:
    ...
    version, commit, shown, out, arches, unsafe = run_guards(root, env)

    if env.get("GUARDS_ONLY", "0") == "1":
        print(
            f"==> GUARDS_ONLY=1: every guard passed; stopping before the Docker build. Would write {shown}",
            file=sys.stderr,
        )
        return harness.EXIT_OK

    return harness.finish(lambda: body(root, version, commit, shown, out, arches, unsafe))
