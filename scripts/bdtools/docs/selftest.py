...
`python3 scripts/bdtools/docs/selftest.py --check-anchors` dry-runs the
anchors against the real tree, building nothing and writing nothing,
which is what `scripts/bdtools/selftest/check_anchors.py` runs.
...
PROMOTED = (
    ...
    "// scripts/bdtools/docs/selftest.py (issue #526). If you are reading this in a real\n"
...
def run_commentsonly(root: Path, only: str) -> tuple[int, str]:
    proc = subprocess.run(
        [sys.executable, str(root / "scripts/bdtools/docs/check_comments_only.py"), "HEAD", "--only", only],
        cwd=root,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
        text=True,
