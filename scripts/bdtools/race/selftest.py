...
in the shared way `scripts/bdtools/compat/selftest.py` and
`scripts/bdtools/conformance/selftest.py` already are, so a refactor
that moves the code cannot leave this control quietly planting nothing.
`python3 scripts/bdtools/race/selftest.py --check-anchors` is that drift
check on its own, and `scripts/bdtools/selftest/check_anchors.py` runs it
...
consumes `scripts/bdtools/selftest_swap.py`'s `AnchorTracker` rather than
reimplementing anchor-matching a second time, exactly the way
`scripts/bdtools/conformance/selftest.py` does.
